...
function ci_comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<fieldset><label for="author">' . __('Name', 'ci_theme') . ': '.$req_string.'</label><input type="text" class="input-text" id="author" name="author" value="' . esc_attr( $commenter['comment_author'] ) . '"' . $aria_req . ' /></fieldset>',
		'email'  => '<fieldset><label for="email">' . __('Email', 'ci_theme') . ': '.$req_string.'</label><input type="text" class="input-text" id="email" name="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '"' . $aria_req . ' /></fieldset>',
		'url'    => '<fieldset><label for="url">' . __('Website', 'ci_theme') . ':</label><input type="text" class="input-text" id="url" name="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" /></fieldset>',
	);
	...
	?>
		<?php if ( comments_open() ) : ?>
			<?php do_action( 'comment_form_before' ); ?>
				...
				<h2 id="reply-title" class="comments"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?><small><?php cancel_comment_reply_link( $args['cancel_reply_link'] ); ?></small></h2>
				<div class="post-form">
